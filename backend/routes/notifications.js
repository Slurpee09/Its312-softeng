...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    // Check if the requesting user is an admin; if so, return admin notifications (new applications)
    const [[userRow]] = await db.query(`SELECT id, role FROM users WHERE id = ? LIMIT 1`, [userId]);
    const isAdmin = userRow && userRow.role === "admin";

    if (isAdmin) {
      // Fetch recent applications (exclude drafts), latest first
      const [apps] = await db.query(
        `SELECT id, program_name, full_name, created_at, UNIX_TIMESTAMP(created_at) AS ts
         FROM applications
         WHERE (status IS NULL OR status != 'Draft') AND created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)
         ORDER BY created_at DESC
         LIMIT 50`
      );

      const notifications = apps.map((a) => ({
        id: a.id,
        application_id: a.id,
        title: `New Application: ${a.program_name || "Unknown"}`,
        message: `${a.full_name || "Unknown applicant"} applied for ${a.program_name || "a program"}`,
        date: a.created_at,
        ts: a.ts,
        type: "application",
        notification_key: `application:${a.id}:${a.ts}`,
      }));

      // get read flags for admin user
      const keys = notifications.map((n) => n.notification_key);
      let readRows = [];
      if (keys.length > 0) {
      ...
      const final = notifications.map((n) => ({ ...n, read: readRows.includes(n.notification_key) }));
      return res.json(final);
    }

    // Non-admin: existing behavior (notifications for user's applications)
